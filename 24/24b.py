...
def find_max_length_bridge(existing_bridge, available_components, pins):
    """
    ...
    """
    max_length_bridge = existing_bridge
    for current_component in available_components:
        if pins in current_component:
            ...
            candidate_bridge = find_max_length_bridge(existing_bridge + [current_component],
                                                      [c for c in available_components if current_component != c],
                                                      new_pins)
            if len(candidate_bridge) >= len(max_length_bridge):
                max_length_bridge = candidate_bridge
            elif len(candidate_bridge) == len(max_length_bridge):
                if bridge_strength(candidate_bridge) > bridge_strength(max_length_bridge):
                    max_length_bridge = candidate_bridge
    return max_length_bridge


print(bridge_strength(find_max_length_bridge([], components, 0)))
